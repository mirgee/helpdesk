<?php //netteCache[01]000402a:2:{s:4:"time";s:21:"0.39738200 1374834635";s:9:"callbacks";a:2:{i:0;a:3:{i:0;a:2:{i:0;s:19:"Nette\Caching\Cache";i:1;s:9:"checkFile";}i:1;s:80:"/var/www/clients/client0/web44/web/helpdesk_nette/helpdesk/libs/Grido/Grid.latte";i:2;i:1374834114;}i:1;a:3:{i:0;a:2:{i:0;s:19:"Nette\Caching\Cache";i:1;s:10:"checkConst";}i:1;s:25:"Nette\Framework::REVISION";i:2;s:30:"dc83a21 released on 2013-07-11";}}}?><?php

// source file: /var/www/clients/client0/web44/web/helpdesk_nette/helpdesk/libs/Grido/Grid.latte

?><?php
// prolog Nette\Latte\Macros\CoreMacros
list($_l, $_g) = Nette\Latte\Macros\CoreMacros::initRuntime($template, 'h35nov0spl')
;
// prolog Nette\Latte\Macros\UIMacros
//
// block _grid
//
if (!function_exists($_l->blocks['_grid'][] = '_lb9701390db6__grid')) { function _lb9701390db6__grid($_l, $_args) { extract($_args); $_control->validateControl('grid')
...
// block outerFilter
//
if (!function_exists($_l->blocks['outerFilter'][] = '_lbc3e0c89d1b_outerFilter')) { function _lbc3e0c89d1b_outerFilter($_l, $_args) { extract($_args)
...
// block table
//
if (!function_exists($_l->blocks['table'][] = '_lbd820a2b5a5_table')) { function _lbd820a2b5a5_table($_l, $_args) { extract($_args)
...
<?php endif ;if ($column->getSort() == Grido\Components\Columns\Column::DESC): ?>
                        <a class="sort" href="<?php echo htmlSpecialChars($_control->link("sort!", array(array($column->getName() => isset($control->defaultSort[$column->getName()]) ? '' : NULL)))) ?>
"><?php echo Nette\Templating\Helpers::escapeHtml($template->translate($column->label), ENT_NOQUOTES) ?></a>
<?php endif ?>
                        <span></span>
<?php else: ?>
...
<?php if (is_string($column->customRender)): Nette\Latte\Macros\CoreMacros::includeTemplate($column->customRender, array('control' => $control, 'presenter' => $control->presenter, 'item' => $row) + $template->getParameters(), $_l->templates['h35nov0spl'])->render() ;else: ?>
...
// block operations
//
if (!function_exists($_l->blocks['operations'][] = '_lbfff793ee7d_operations')) { function _lbfff793ee7d_operations($_l, $_args) { extract($_args)
...
// block paginator
//
if (!function_exists($_l->blocks['paginator'][] = '_lb494112fb64_paginator')) { function _lb494112fb64_paginator($_l, $_args) { extract($_args)
...
// block count
//
if (!function_exists($_l->blocks['count'][] = '_lb1f97b7d617_count')) { function _lb1f97b7d617_count($_l, $_args) { extract($_args)
?>                <span class="count">
                    <?php echo Nette\Templating\Helpers::escapeHtml(sprintf($template->translate('Items %d - %d of %d'), $paginator->getCountBegin(), $paginator->getCountEnd(), $control->count), ENT_NOQUOTES) ?>
...
<?php $_input = (is_object("count") ? "count" : $_form["count"]); echo $_input->getControl()->addAttributes(array()) ;$_formStack[] = $_form; $formContainer = $_form = (is_object("buttons") ? "buttons" : $_form["buttons"]) ;$_input = (is_object("perPage") ? "perPage" : $_form["perPage"]); echo $_input->getControl()->addAttributes(array('class' => 'hide')) ;$_form = array_pop($_formStack) ;if ($control->hasExporting()): ?>
                    <a class="btn btn-mini no-ajax" href="<?php echo htmlSpecialChars($control->getComponent(Grido\Components\Export::ID)->link('export!')) ?>
" title="<?php echo htmlSpecialChars($template->translate('Export all items')) ?>"><i class="icon-download"></i></a>
<?php endif ?>
                </span>
<?php
}}
